...
STARTING_NUMBER_OF_EACH_MACHINE = [4,7,11,20,5,4,26,5,3,20]
...
def optimize(repetitions=5, runs=5, numbers_of_each_machine=[2,6,11,20,5,4,26,5,3,20], target=1000):
    SPEC_PATH = "./Machine_Specs.csv"
    specs = pd.read_csv(SPEC_PATH, index_col="Item",)
    machine_names = specs.index
    reversed_names = machine_names[::-1]
    finished_by_type = {}
    # this converts each machines input units in terms of the end result of the entire process
    expected_output_percents = {}
    yield_ratio = 1
    prev_name = None
    # use the expected yield of each step to know how many pounds you need to get the target
    for name in reversed_names:
        if prev_name:
            mtbf = specs.loc[prev_name, "MBTF-Days"] * WORK_HOURS_PER_DAY
            mttr = specs.loc[prev_name, "MMT-Days"] * WORK_HOURS_PER_DAY
        else:
            mtbf = 1
            mttr = 0
        batch_success_rate = 1 - BATCH_FAILURE_RATE
        avail = mtbf/(mtbf+mttr)
        yield_percent = float(specs.loc[name, "Yield"][:2])/100
        yield_ratio *= batch_success_rate * yield_percent
        # We put availability here, because only the bottleneck will be limited by availability
        # Therefore we do not want to keep a running tally like we do with yield
        expected_output_percents[name] = yield_ratio * avail
        prev_name = name
        print(f'{name} yield ratio is {expected_output_percents[name]} with avail of {avail}, batch succes of {batch_success_rate} and yield of {yield_percent}')
    ...
    for i in range(repetitions):
        ...
        for j in range(runs):
            ...
            for name in machine_names:
                finished_by_type[name] += expected_output_percents[name] * sum([machine.number_finished for machine in machine_dict[name]]) /(2000 * runs)
...
print(optimize(3,3, STARTING_NUMBER_OF_EACH_MACHINE))
